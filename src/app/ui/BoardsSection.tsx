    ...
    <InViewContainer>
      <div className="h-full bg-main-brown-default bg-opacity-80 p-4 flex justify-center flex-wrap gap-5 content-between md:content-center py-4">
        {menu.filter(l=>l.text!="Home").map((item) => {
          return (
              <Link key={item.link} href={item.link} className="hover:scale-125 md:w-1/5 w-3/4 md:h-3/4 h-[30%] min-h-56 p-4 bg-surfboard bg-center bg-contain bg-no-repeat flex flex-col justify-center  transition-all  ">
            <div  >
                <Header
                  style={{
                    textShadow:
